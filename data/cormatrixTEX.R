...
# This function below essentially creates a humongous correlation table
...
corstarsl <- function(x, long = FALSE) {
  ...
  mystars <- ifelse(p < .001, "***", ifelse(p < .01, "** ",
                                            ifelse(p < .05, "* ", " ")))
 
  ## trunctuate the matrix that holds the correlations to two decimal
  R <- format(round(cbind(rep(-1.11, ncol(x)), R), 3))[,-1]
 
  ## build a new matrix with the correlations with their apropriate stars
  ...
  if (long == TRUE) {
    return(list(data.frame(t(Rnew), stringsAsFactors = FALSE), 
                round(p, digits = 6)))
  } else {
    return(Rnew)
  }
